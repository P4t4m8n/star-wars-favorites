...
import DetailsPlanetModalInfo from "./DetailsPlanetModalInfo";

function DetailsPlanet({ item }) {
    ...
    return (
        <>
            ...
            <div className='item-details-lists grid currTheme'>
                ...
                {(characters && characters.length > 0) && <div className="details-list-container flex flex-column">
                    <h3>Characters: </h3>
                    <DetailsItemList items={characters} />
                </div>}

            </div>
            {open && <DetailsModal setOpen={setOpen} children={<DetailsPlanetModalInfo item={item} />} />}
        </>
    )
}
